...
cost_function <- function(alpha, beta, x, y){
  # The error is the difference between the real value y at x and the predicted value alpha + beta*x
...
gradient_desc <- function(alpha, beta, x, y){
  grad_beta <- -2 * x * (y - (alpha + beta*x)) # -cost'(beta)
  return (grad_beta)
}
```
...
minimize_function <- function(alpha, beta, x, y, delta){
  new_beta <- beta - delta * mean(gradient_desc(alpha, beta, x, y))
  return(new_beta)
}
```
...
res <- res %>% bind_cols(loss) %>%
  mutate(iteration = seq(1, iter)) %>% as_tibble()
