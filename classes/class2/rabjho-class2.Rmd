...
knitr::opts_knit$set(root.dir = "C:\\Users\\Rabjho\\OneDrive\ -\ Aarhus\ universitet\\Methods 2\\methods02-cogsci\\classes")
...
while (number < 20) {
  print("the number is below 20, so I still run! (until it isn't true anymore)")
  print(number)
  number = number + 7
...
basketball <- function(printing = FALSE, shots = character(0)) {
    # starting the function for you
    shots <- c(sample(c("hit", "miss"), 1, prob = c(0.6, 0.4), replace = TRUE), shots)
    if (length(shots) >= 2 && shots[1] == "miss" && shots[2] == "miss") {
      if (printing) {
        print(shots)
      }
      return(shots)
    ...
    return(c(basketball(printing = printing, shots=shots), shots))
